...
export interface BookingPayload {
  ...
  date: string;
  startTime: string;
  services: { serviceId: string; userId: string }[];
...
export default function BookingForm({
  ...
  // Form State
  const [servicesSelected, setServicesSelected] = useState<string[]>(initialData?.servicesSelected || []);
  const [date, setDate] = useState<string>(initialData?.date || "");
  const [time, setTime] = useState<string>(initialData?.time || "");
  const [serviceUsers, setServiceUsers] = useState<Record<string, string>>(initialData?.serviceUsers || {});
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 sm:px-6 lg:px-8 text-center">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Ingresa para Agendar o Editar</h2>
          <p className="text-slate-600 mb-8 max-w-md mx-auto">
            Para poder gestionar tus citas, necesitas crear una cuenta o iniciar sesión.
          </p>
      ...
      <div className="mt-6 bg-slate-50 rounded-xl p-4 border border-slate-100">
        ...
        <ul className="text-sm text-slate-600 space-y-1">
          {days.map((day) => {
            ...
            return (
              <li key={day.key} className="flex justify-between border-b border-slate-200 pb-1 last:border-0 last:pb-0">
                <span>{day.label}</span>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="pt-6">
                <button
                  ...
                  className="w-full flex justify-center items-center gap-2 py-4 px-4 border border-transparent rounded-xl shadow-sm text-lg font-bold text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors disabled:opacity-70"
                >
